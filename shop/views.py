...
from shop.models import Item_Category
...
def categories_list():
    #categories
    categories_list = Item_Category.objects.filter(is_active=True).distinct()
...
class HomeView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update(categories_list())
        return context
...
class CategoryListView(ListView):
    ...
    def get_queryset(self):
        #Menu Model Class return use By slug
        self.menu = get_object_or_404(Menu, slug=self.kwargs.get(self.name_of_slug))
        queryset =Item.objects.filter(is_active=True, category__is_active=True).order_by('-updated_on')
        return queryset
...
class ProfileView(View):
    def get(self, *args, **kwargs):
        ...
        return render(self.request, "shop/profilepage.html", context)
